...
';
    ...
    function confirmed_ticket() {
	 ...
	 function confirmed_ticket_json() {
        if ($this->session->userdata('bktravels_logged_in') != TRUE) {
            redirect(get_cookie('website_url'));
        } else {
            $this->booking_m->confirmed_ticket_db_json();
        }
    }
	function sendSms() {
        if ($this->session->userdata('bktravels_logged_in') != TRUE) {
	...
	function junoresponse() {        
		$mtxnid = $this->input->post('mtxnid');
        $jtxnid = $this->input->post('jtxnid');        
		$status = $this->input->post('status');
        //echo $mtxnid.'#'.$jtxnid.'#'.$status;
        if (strtolower($status) == 'success') {
			$result = $this->booking_m->junoresponse1($mtxnid);			
            $this->db->query("update master_booking set authid='$jtxnid' where refno='$mtxnid' and pay='junosms'");
			echo "<script>javascript:alert('Your Transaction is Successfull, Will receive Confirmation SMS Shortly Thank You');</script>";
			$onward_tktno = $result['onward_tktno'];
			echo '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1" />
...
<body onLoad="document.confirmed_ticket.submit()">
<form name="confirmed_ticket" method="get" action="' . base_url('booking/juno_confirmed_ticket') . '">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td>
    <input name="onward_tktno" type="hidden" value="' . $onward_tktno . '" />
	<input name="onward_way" type="hidden" value="O" />    
    </td>
  </tr>
</table>
</form>
...
';
			...
			$result = $this->booking_m->juno_release_seats1($mtxnid);
            echo "<script>javascript:alert('Transaction Not Successful Please Try Again');</script>"; 
			echo "<script>window.close();</script>";   
			echo $status;        
        }        
    }
	
	function juno_confirmed_ticket() {        
        $this->booking_m->juno_confirmed_ticket_db();        
    }
