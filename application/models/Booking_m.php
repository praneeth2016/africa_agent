...
<!--img class="callout1" src="' . base_url("images/callout_black4.gif") . '" /-->';
        if ($seat_status == 2) {
            ...
            if ($seat_status == 3) {
                echo'<tr>
  <td height="25" colspan="3" align="left"><strong>' . $agName . '</strong> (' . $time . ') Pending</td>
  </tr>';
            } else if ($seat_status == 1) {
                echo'<tr>
  <td height="25" colspan="3" align="left"><strong>' . $agName . '</strong> (' . $time . ') Confirmed</td>
  </tr>';
            }
            echo'<tr>
  ...
  <tr>
    ...
    <td width="10"><strong>:</strong></td>
    <td height="25" align="left">' . $passname . ' - ' . $pmobile . '</td>
  </tr>
  <tr>
  ...
  <!--tr>
    <td height="25" colspan="3" align="center" ><button class="tooltip-btn-prnt" onClick="getPrint(\''.$seatname.'\',\''.$serno.'\',\'' .$jdate.'\')" >Print</button>&nbsp;
	<button type="button" onClick="getPrint(\''.$seatname.'\',\''.$serno.'\',\'' .$jdate.'\')" class="tooltip-btn-cnl">Cancel</button></td>  
  </tr-->
</table>';
        }
    }
    ...
    function show_paytype() {
        ...
        if ($paytyp == 'byagent' || $paytyp == 'byphoneagent') {
            ...
            //echo $service_num."#".$source_id."#".$travel_id."#".$lm;
            echo '<p style="float: left;">Agent Name : </p><label style="width: 25%;">
			<select name="pay_agent" id="pay_agent" ng-model="saveTicket.pay_agent" style="width: 100%;" required="required">
				<option value="0">-Select-</option>';
				$stmt = "select distinct appname,name,id from agents_operator where operator_id='$travel_id' and agent_type='2' and status='1'";
				$query = $this->db->query($stmt);
				...
				echo '</select></label>             
                <label>Reciept No : <input type="text" required ng-model="saveTicket.receiptno" style="width: 65%;border:1px solid #337ab7;" name="receiptno" id="receiptno" /></label>';
        } else if ($paytyp == 'byemployee') {
            //echo $service_num."#".$source_id."#".$travel_id."#".$lm;
            echo '<p style="float: left;">Employee Name : </p>
			<label style="width: 25%;">
			<select name="pay_agent" id="pay_agent" ng-model="saveTicket.pay_agent" style="width: 100%;" required="required">
				<option value="0">-Select-</option>';
				$stmt = "select distinct appname,name,id from agents_operator where operator_id='$travel_id' and agent_type='1' and status='1'";
				$query = $this->db->query($stmt);
                ...
                if ($onward_way == "O") {
                    ...
                    if (count(array_unique($statusinfo)) === 1 && (end($statusinfo) === '2' || end($statusinfo) === '3')) {                        
                        ...
                        if ($paytyp == "byphone") {
                            ...
                            $this->db->query("insert into master_proceed_details(travel_id,custname,ip,refno,email,mobile,source,dest,jdate,bdate,pass,
							fare,paid,brdpt,drpt,seatno,tim,altph,service_no,tktno,pnr,book_pay_type,book_pay_agent) values('$onward_travel_id',
							'$onward_names','$ip','$onward_refno','$email','$mobile','$onward_source_name','$onward_destination_name','$reqphone',
							'$booked_date','$onward_pass','$onward_fare','$onward_fare','$onward_bp','$onward_dp','$onward_seats','$booked_time',
							'','$onward_service_num','$onward_tktno','$onward_pnr','$paytyp','$book_pay_agent')") or die(mysql_error());

                            //inserting master booking details..				
                            $sql1 = $this->db->query("insert into master_booking(tkt_no,pnr,service_no,board_point,bpid,land_mark,dpid,drop_point,source,
							dest,travels,bus_type,bdate,jdate,seats,gender,start_time,arr_time,paid,save,tkt_fare,base_fare,service_tax_amount,
							discount_amount,convenience_charge,pname,pemail,pmobile,age,refno,status,pass,travel_id,ip,time,id_type,id_num,padd,
							alter_ph,fid,tid,operator_agent_type,agent_id,bus_model,book_pay_type,book_pay_agent) values('$onward_tktno','$onward_pnr',
							'$onward_service_num','$onward_bp','$onward_bpid','$onward_lm','$onward_dpid','$onward_dp','$onward_source_name',
							'$onward_destination_name','$onward_travels','$onward_bus_type','$booked_date','$reqphone','$onward_seats','$onward_genders',
							'$onward_start_time','$onward_arr_time','$paid','$save','$onward_fare','$base_fare','$onward_service_tax_amount',
							'$onward_discount_amount','$onward_convenience_charge','$onward_names','$email','$mobile','$onward_ages','$onward_refno',
							'pending','$onward_pass','$onward_travel_id','$ip','$booked_time','$cardtype','$cardnum','$address','$altph','$onward_source_id',
							'$onward_destination_id','$agent_type','$agentid','$onward_model','$paytyp','$book_pay_agent')") or die(mysql_error());
                            ...
                            $text = urlencode("PNR " . $onward_pnr . " " . $onward_source_name . "-" . $onward_destination_name . " on " . $reqphone . " for seats " . $onward_seats . " in " . $onward_travels . " of amount " . $onward_fare . " is pending, Pay before " . $msg_time . " to confirm ticket.");
							
							$this->booking_m->msg91sms($mobile,$senid,$text);
                            ...
                            $text = urlencode("TKT No: " . $onward_tktno . "->" . $onward_travels . "-" . $onward_source_name . "-" . $onward_destination_name . "->" . $onward_service_num . " , DOJ: " . $onward_date . " , Seats: " . $onward_seats . " , At-" . $onward_bp . " , Ph: " . $ph . "");
							$this->booking_m->msg91sms($mobile,$senid,$text);
                            /*$ch = curl_init();
                            curl_setopt($ch, CURLOPT_URL, "http://api.mVaayoo.com/mvaayooapi/MessageCompose");
                            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                            ...
                            //$img = "http://ticketengine.in/intl_operator_logo/' . $onward_travel_id . '.png"
                            //}
...
<table align="center" border="0" cellpadding="2" cellspacing="0" width="100%">
  <tbody>
	<tr align="left" valign="top">
	  ...
	  <img src="http://ticketengine.in/intl_operator_logo/' . $onward_travel_id . '.png" />
	  	</td>
    </tr>
	<tr align="left" valign="top">
	  <td colspan="6"style="color:#c42124;padding-left:5px;font-size:13px;">&nbsp;
...
';
	...
	function sendSms()
	{
		$onward_tktno = $this->input->post('onward_tktno');
		//echo $onward_tktno."before===";
			...
			$sql5 = $this->db->query("SELECT distinct op_url,op_email,other_contact,canc_terms FROM registered_operators where travel_id='$onward_travel_id'");
			foreach ($sql5->result() as $row5) {
				$op_url = $row5->op_url;
				$op_email = $row5->op_email;
			...
			/*$headers = "MIME-Version: 1.0" . "\r\n";
			$headers .= "Content-Type: text/html; charset=ISO-8859-1" . "\r\n";
			$headers .= 'From: TEINTL <info@ticketengine.in>' . "\r\n";
			$headers .= 'Reply-To: info@ticketengine.in' . "\r\n";*/
...
<table align="center" border="0" cellpadding="2" cellspacing="0" width="100%">
  <tbody>
    ...
    <td colspan="3" style="color:#c42124;padding-left:5px;font-size:13px;" align="left"><strong>Ticket No : ' . $referenceno . ' </strong></td>
    </tr>
    <tr align="center" valign="top">
    <td colspan="6"><strong></strong></td>
    </tr>
...
mail($pemail, $subject, $message, $headers);
		...
		//echo $tkt_no;
	//}
	function confirmed_ticket_db_json()
	{
...
<table width="100%" height="214">
  <tr>
    ...
    <td width="393" valign="top">Ticket No : &nbsp; &nbsp;<strong>' . $pnr . '</strong>(' . $onward_tktno . ')</td>
  </tr>
  <tr>
  ...
  <tr>
    <td align="center">
      <table width="100%" border="0" cellspacing="1" cellpadding="1" >
        <tr>
          <td height="30" colspan="2" align="center"><img src="http://ticketengine.in/intl_operator_logo/' . $travel_id . '.png"  alt="' . $travels . '" width="160" height="50" /></td>
        </tr>
        <tr>
...
<div align="center">';
    ...
    function layout_change_price2() {
        ...
        echo '<table width="545" border="0" align="center" >
		     <tr >
				...
				<td width="137"><input type="text" name="fdate" readonly="" class="inputmedium form-control" id="fdate" value="' . $journey_date . '"   /></td>
				
				<td width="29">&nbsp;</td>
				<td width="81">To    Date: </td>
				<td width="181"><input type="text" name="tdate" readonly="" class="inputmedium form-control" id="tdate"   value="' . $journey_date . '"/></td>
			</tr>
		</table>';
  ...
  <tr>
    <td height="30" align="center">' . $service_name . '</td>';
            ...
            if ($bus_type == "seater") {
                echo'<td height="30" align="center"><input type="text" class="" name="sfare' . $i . '" id="sfare' . $i . '" value="' . $sfare . '"></td>';
            } elseif ($bus_type == "sleeper") {
                echo'<td height="30" align="center"><input type="text" class="" name="lbfare' . $i . '" id="lbfare' . $i . '" value="' . $lfare . '"></td>
    <td height="30" align="center"><input type="text" class="" name="ubfare' . $i . '" id="ubfare' . $i . '" value="' . $ufare . '"></td>';
            } else {
                echo'<td height="30" align="center"><input type="text" class="" name="sfare' . $i . '" id="sfare' . $i . '" value="' . $sfare . '"></td>
    <td height="30" align="center"><input type="text" class="" name="lbfare' . $i . '" id="lbfare' . $i . '" value="' . $lfare . '"></td>
    <td height="30" align="center"><input type="text" class="" name="ubfare' . $i . '" id="ubfare' . $i . '" value="' . $ufare . '"></td>';
            }
            echo'</tr>
  <tr>
...
';
  ...
  <tr style="background-color:#CCCCCC">
    ...
    <td height="30" align="center"><input type="text" class="" name="ubfare' . $i . '" id="ubfare' . $i . '" value="' . $ufare . '"></td>';
                    ...
                    echo'<td height="30" align="center"><input type="text" class="form-control" name="sfare' . $i . '" id="sfare' . $i . '" value="' . $sfare . '"></td>
    <td height="30" align="center"><input type="text" class="" name="lbfare' . $i . '" id="lbfare' . $i . '" value="' . $lfare . '"></td>
    <td height="30" align="center"><input type="text" class="" name="ubfare' . $i . '" id="ubfare' . $i . '" value="' . $ufare . '"></td>';
                }
                echo'</tr>';
...
<table width="100%" height="214">
  <tr>
    ...
    <td width="393" valign="top">Ticket No : &nbsp; &nbsp;<strong>' . $pnr . '</strong>(' . $tkt_no . ')</td>
  </tr>
  <tr>
    <td width="231">Mobile : &nbsp; &nbsp; '.$pmobile.'</td>
    <td width="197">'.$idtype2.' No:  &nbsp; &nbsp; '.$id_num.'</td>
    ...
    function canc_ticket1() {
        ...
        if ($stat != "cancelled") {
            ...
            if ($sql->num_rows() != 0) {
                ...
                $this->db->query("insert into master_booking(tkt_no,pnr,service_no,board_point,bpid,land_mark,source,dest,travels,bus_type,bdate,jdate,seats,gender,start_time,arr_time,paid,save,tkt_fare,base_fare,service_tax_amount,discount_amount,convenience_charge,pname,pemail,pmobile,age,refno,status,pass,cseat,ccharge,camt,refamt,travel_id,ip,time,cdate,ctime,id_type,id_num,padd,alter_ph,fid,tid,operator_agent_type,agent_id,book_pay_type,book_pay_agent,cancelled_by,cgst,sgst)
				values('$tkt_no','$pnr','$service_no','$board_point','$bpid','$land_mark','$source','$dest','$travels','$bus_type','$bdate','$jdate','$seats','$gender','$start_time','$arr_time','$paid','$save','$tkt_fare','$base_fare','$service_tax_amount','$discount_amount','$convenience_charge','$pname','$pemail','$pmobile','$age','$refno','$status','$pass','$seats','$ccharge','$camt','$refamt','$travel_id','$ip','$time','$cdate','$ctime','$id_type','$id_num','$padd','$alter_ph','$fid','$tid','$operator_agent_type','$agent_id1','$book_pay_type','$book_pay_agent','$agent_id','$cgst','$sgst')")or die(mysql_error());
				...
				//$this->booking_m->msg91sms($pmobile,$sender_id,$text);
                //echo $text;
				...
				*/
					$receipientno = $pmobile;
					$ch = curl_init();
					//curl_setopt($ch, CURLOPT_URL, "http://localhost:8080/TestSpring/updateCancelTicket/$travel_id/$tkt_no");
					curl_setopt($ch, CURLOPT_URL, "http://ticketengine.in:8080/TestSpring_intl_live/updateCancelTicket/$travel_id/$tkt_no");
					//curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
					//curl_setopt($ch, CURLOPT_GET, 1);
					//curl_setopt($ch, CURLOPT_POSTFIELDS, "operatorId=$travel_id&tktno=$tktno");
					$buffer = curl_exec($ch);
					curl_close($ch);
				  $this->booking_m->msg91sms($receipientno,$text,$iso,$dialCode);
            }
        }
    ...
    function confirm_ticket1() {
        ...
        foreach ($sql->result() as $row) {
            ...
            if ($status == "pending") {
				...
				$this->booking_m->msg91sms($pmobile,$senid,$text);
                /*$ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, "http://api.mVaayoo.com/mvaayooapi/MessageCompose");
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($ch, CURLOPT_POST, 1);
